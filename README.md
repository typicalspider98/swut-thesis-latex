...
1. 修改 `main.tex` 第 2 区的论文题目、作者、学院、专业等信息。
   封面类型不设置时保留“毕业设计（论文）”；使用
   `\thesistype{sheji}` 显示“毕业设计”，使用
   `\thesistype{lunwen}` 显示“毕业论文”。
...
- **v0.2.11（2026-08-01）**：新增封面类型命令
  `\thesistype{sheji}` 和 `\thesistype{lunwen}`，分别显示
  “毕业设计”和“毕业论文”；不使用该命令时继续显示
  “毕业设计（论文）”，保持现有文档兼容。
...
- **v0.2.8（2026-07-29）**：优化封面信息区的垂直对齐，使学生姓名、
  学号、学院、专业及指导教师更靠近各自下划线，并保持双导师姓名的
  紧凑行距。
- **v0.2.7（2026-07-29）**：放宽大图在页面顶部与后续正文共同排版的
  浮动比例；当前页空间不足时，图片或表格可移至下一页顶部，后续正文
  接排在其下方，不再无故推迟到第三页，同时仍保持图表与正文的源文件
  顺序。
- **v0.2.6（2026-07-29）**：代码块恢复为正文流中的可跨页环境，长代码
  自动连续换页，不再被拆成居中的半页浮动块；统一代码块与前后正文
  的间距。
